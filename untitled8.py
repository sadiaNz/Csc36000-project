...
Original file is located at
    https://colab.research.google.com/drive/1GWOgc9L0vKZwhZcw09lvNnCzoL0rvnjd
"""
...
def run_hlscsn_ablation(seed=0, episodes=2000):
    ...
    # -----------------------------------------------
    #  Re-run EXACTLY the same code as your ablation!
    #  But in smaller form: do not print, only store.
    # -----------------------------------------------
    ...
    for ep in range(episodes):
        ...
        # Rewards
        def compute_reward(res):
            if res["aligned"] and res["decision"] == "CONTINUE":
                return 2
            elif res["aligned"] and res["decision"] == "NEGOTIATE":
                return 1
            elif (not res["aligned"]) and res["decision"] == "NEGOTIATE":
                return 0
            else:
                return -2

        R_full.append(compute_reward(r_full))
        R_no.append(compute_reward(r_no))
        R_always.append(compute_reward(r_always))
...
# ================================
# Combined SAR, NE, TCR Evolution Plot
# ================================

# set x-axis scale to match PPO 50 000 training steps
total_steps = 50000
steps = np.linspace(0, total_steps, len(SAR_list))


plt.figure(figsize=(14, 7))

# Plot SAR
sar_smooth = smooth(SAR_list, 200)
plt.plot(steps, sar_smooth, color="#E69F00", linewidth=3, label="Semantic Alignment Rate (SAR)")
plt.fill_between(steps, sar_smooth - 0.05, sar_smooth + 0.05, color="#E69F00", alpha=0.2)

# NE horizontal reference line
plt.axhline(np.mean(NE_list), linestyle="--", color="red",
            label=f"Negotiation Efficiency (NE) = {np.mean(NE_list):.3f}")

# TCR horizontal reference line
plt.axhline(np.mean(TCR_list), linestyle="-.", color="green",
            label=f"Task Continuity Ratio (TCR) = {np.mean(TCR_list):.3f}")
...
print(f"SAR={SAR:.3f}, NE={NE:.3f}, TCR={TCR:.3f}")

"""**PLOTS FOR HLSCSN ABLATION**"""

# ===============================================================
# STEP 5A – RESEARCH-LEVEL PLOTS FOR HLSCSN ABLATION
# ===============================================================
...
def plot_SAR_with_jitter():
    plt.figure(figsize=(10,6))

    for mode in modes:
        data = np.array(results[mode]["SAR"])

        # Add tiny jitter so lines do not overlap visually
        jitter = {
            "full": 0.0002,
            "no_neg": -0.0002,
            "always": 0.0
        ...
        # very small gap so lines become visible
        gap = {
            "full": 0.0025,      # shift FULL slightly upward
            "no_neg": -0.0025,   # shift NO_NEG slightly downward
            "always": 0.00        # keep ALWAYS baseline
        }[mode]
    ...
    plt.title("SAR Across Seeds (Separated for Visualization)", fontsize=14)
    plt.xlabel("Seed Index")
    plt.ylabel("Semantic Alignment Rate")
    plt.legend()
    plt.grid(True)
    plt.show()

plot_SAR_with_jitter()

"""**Ablation Curves (Main Result)**

Figure 1. Ablation comparison of semantic alignment.
Alignment rate (SAR) across 2000 episodes for three HLSCSN variants.
The FULL model maintains consistently higher alignment than NO-NEG and ALWAYS-NEG, demonstrating the value of the Bayesian+RL negotiation rule.

**FIGURE 2 — Multi-Seed Mean ± Std Bar Chart**

Figure 2. Multi-seed performance summary (mean ± standard deviation).
Across 5 independent seeds, the FULL model achieves higher average alignment (SAR), moderate negotiation effort (NE), and strong task continuity (TCR) compared to ablation baselines, indicating stable and reproducible semantic behavior.
"""
